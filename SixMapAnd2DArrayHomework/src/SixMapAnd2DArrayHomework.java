...
public class SixMapAnd2DArrayHomework {
	...
	public static HashMap<String,ArrayList<Integer>> reverseHashmap(HashMap<Integer,String> initialMap) {
		...
		for(int key : initialInts){
			if(reverseMap.containsKey(initialMap.get(key))) {
				reverseMap.get(initialMap.get(key)).add(key);
			}else {
	...
	public static String getCommonMother(HashMap<String,String> motherMap, String person1, String person2) {
		ArrayList<String> motherTree1 = new ArrayList<String>();
		ArrayList<String> motherTree2 = new ArrayList<String>();
		String mother1 = person1;
		String mother2 = person2;
		while(mother1 != null) {
			motherTree1.add(mother1);
			mother1 = motherMap.get(mother1);
		}
		motherTree1.add(null);
		while(mother2 != null) {
			motherTree2.add(mother2);
			mother2 = motherMap.get(mother2);
		}
		motherTree2.add(null);
		String commonMother = null;
		int i = 1;
		while(true) {
			if(motherTree1.get(motherTree1.size() - i) != motherTree2.get(motherTree2.size() - i)) {
				break;
			}
			commonMother = motherTree1.get(motherTree1.size() - i);
			i++;
			if(motherTree1.size() - i < 0 || motherTree2.size() - i < 0) {
				break;
			}
		}
		return commonMother; 
	...
	public static char[][] distanceArray(int row, int col, int distance) {
		...
		for(int i = 0; i < 10; i++) {
			for(int j = 0; j < 10; j++){
				int manDistance = Math.abs(row - i) + Math.abs(col - j);
