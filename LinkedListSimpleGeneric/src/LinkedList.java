...
public class LinkedList<DataType> {
	 ...
	 * @param value
	 */
	public void addAtBeginning(DataType value) {
		this.head = new Node<DataType>(value,this.head);
	}
	 ...
	 * @param value
	 */
	public void addAtEnd(DataType value) {
		if(this.head == null) {
		...
		current.next = new Node<DataType>(value, null);
	}
	 ...
	 */
	public void addAfterValue(DataType valueToAdd, DataType valueToInsertAfter) {
		Node<DataType> current = this.head;
		while(current.value != valueToInsertAfter) {
			current = current.next;
		}
		current.next = new Node<DataType>(valueToAdd,current.next);
	}
	 ...
	 */
	public void addBeforeValue(DataType valueToAdd, DataType valueToInsertBefore) {
		if(this.head.value == valueToInsertBefore) {
			addAtBeginning(valueToAdd);
			return;
		}
		Node<DataType> prev = this.head;
		Node<DataType> current = this.head.next;
		while(current.value != valueToInsertBefore) {
			current = current.next;
			prev = prev.next;
		}
		prev.next = new Node<DataType>(valueToAdd,current);
	...
	public LinkedList(DataType[] data) {
		this.head = new Node<DataType>(data[0],null);
		Node<DataType> current = this.head;
		for(int i = 1; i < data.length; i++) {
			current.next = new Node<DataType>(data[i],null);
			current = current.next;
		}
	}
	 ...
	 */
	public void removeAfter(DataType valueToRemoveAfter) {
		Node<DataType> current = this.head;
	 ...
	 */
	public void removalAll(DataType nodesWithValue) {
